...
<style>
  ...
  /* contador de productos junto al Woo ID */
  #catTree .count-badge{
    ...
    border:1px solid var(--line-strong, #aab4d2);
    border-radius:.35rem;
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const RUTA_DELETE = @json(route('categorias.api.delete', ['cliente' => $cliente]));
  ...
  function decorateRow(inst, nodeId){
    ...
    const count = parseInt($li.data('count') ?? 0, 10); // NUEVO
    ...
    // Woo ID + badge de conteo
    $a.append('<div class="cell-woo">'+ wid +
                '<span class="count-badge" title="Productos asociados">'+ count +'</span>' +
              '</div>');
  ...
  function renderNodeTools(inst, nodeId){
    ...
    // Eliminar (solo si NO tiene hijos y NO tiene productos)
    if (!hasChildren && count === 0){
      $actions.append('<button class="btn btn-outline-danger btn-xs btn-del-cat" title="Eliminar categoría"><i class="bi bi-trash"></i></button>');
    }
  }
  ...
  // Eliminar categoría (delegado)
  $(document).on('click', '.btn-del-cat', function (ev) {
    ev.preventDefault();
    const inst = $treeEl.jstree(true);
    const $li  = $(this).closest('.jstree-node');
    const id   = $li.attr('id');
    const name = inst.get_text(id);

    Swal.fire({
      icon:'warning',
      title:'Eliminar categoría',
      html:'¿Eliminar <b>'+ name +'</b>? Esta acción no se puede deshacer.',
      ...
      $.post(RUTA_DELETE, { id })
        .done(() => {
          Swal.close();
          inst.delete_node(id);
          Swal.fire({toast:true,icon:'success',title:'Categoría eliminada',timer:1600,showConfirmButton:false,position:'top-end'});
        })
        .fail(xhr => {
          Swal.close();
          Swal.fire({icon:'error',title:'No se pudo eliminar',text: xhr.responseJSON?.msg ?? xhr.responseJSON?.error ?? 'Error'});
        });
    });
  });
